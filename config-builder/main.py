...
# CFG GENERATOR
# ============================================================================
# TODO Phase 2: Move into core/cfg_generator.py.
# TODO Phase 2: Optional metadata header (creation date, generator version,
#               profile name, hostname) is implemented below as a simple
#               flag; extend with template-based header in a later phase.
...
def generate_cfg(values: dict[str, Any], profile_name: str = "custom",
    ...
    if with_header:
        ...
        lines.append(f"// Generated by Config Builder v{__version__}")
        lines.append(f"// Profile:    {profile_name}")
        lines.append(f"// Created:    {datetime.now().isoformat(timespec='seconds')}")
        lines.append("// =====================================================")
        lines.append("")

    # TODO Phase 3: emit categories in the order from CATEGORIES with
    # section header comments above each block.
    for cvar, value in values.items():
        meta = CVARS.get(cvar)
        if meta is None:
            # Unknown cvar — write it raw with a warning comment. The user
            # may have imported a cfg containing cvars we don't know about
            # yet; don't drop them silently.
